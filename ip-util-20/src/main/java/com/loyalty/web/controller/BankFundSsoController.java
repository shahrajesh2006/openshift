...
import com.loyalty.saml.test.utils.BFSSamlTestHelper;
import com.loyalty.web.constants.Constants;

@Controller
public class BankFundSsoController {
	
	final static Logger log = LoggerFactory.getLogger(BankFundSsoController.class);

	@Autowired
	DBUtil DBUtil;
	
	@RequestMapping(value = "/BankFundSsoAction", method = RequestMethod.POST)
	public String postValues(HttpServletRequest request, HttpServletResponse response)
			throws IOException, ServletException {
		log.debug("Inside BankFundSsoAction - postValues - start");

		// Security Check
		String environment = getEnvironment(request);
		// Commenting to allow prod jump page for QA testing. Uncomment this
		// after CBKC launch
		// Do not allow sso jump page testing in prod
		/*
		 * if ("prod2".equalsIgnoreCase(environment)) { return
		 * mapping.findForward("doSsoLogin.error"); }
		 */

		String accountNumber = request.getParameter("accountNumber");
		String authURL = request.getParameter("authURL");
		String encryptCertAlias = request.getParameter("encryptCertAlias");
		String token = BFSSamlTestHelper.createSamlToken(accountNumber, authURL, encryptCertAlias);
		String ssoUrl = request.getParameter("authURL");
		request.setAttribute("samlToken", token);
		request.setAttribute("ssoUrl", ssoUrl);

		log.debug("Inside BankFundSsoAction - postValues - values == " + accountNumber + " " + authURL + " "
				+ encryptCertAlias + " " + token + " " + ssoUrl);

		return "BankFundSamlSubmit";

...
public final class SAMLConstants {
	 ...
	 * <code>toString(includeNS, declareNS)</code> when <code>declareNS</code>
	 * is true for any assertion element.
	 */
	 ...
	 */
	public static final String X509SUBJECTNAME = assertionSAMLNameSpaceURI + "#X509SubjectName";

	/**
	 * String used in the <code>ActionNamespace</code> attribute to refer to
	 * common sets of actions to perform on resources.
	 *
	 * Title: Read/Write/Execute/Delete/Control Defined actions:
	 * <code>Read Write Execute Delete Control</code> These actions are
	 * interpreted in the normal manner, i.e.
	 * <ul>
	 ...
	 * common sets of actions to perform on resources.
	 *
	 * Title: Read/Write/Execute/Delete/Control with Negation Defined actions:
	 ...
	 * <li><code>~Control</code>: The subject may NOT specify the access control
	 * policy for the resource</li>
	 * </ul>
	 ...
	 * common sets of actions to perform on resources.
	 *
	 * Title: <code>Get/Head/Put/Post</code> Defined actions:
	 * <code>GET HEAD PUT POST</code> These actions bind to the corresponding
	 * HTTP operations. For example a subject authorized to perform the GET
	 * action on a resource is authorized to retrieve it. The GET and HEAD
	 * actions loosely correspond to the conventional read permission and the
	 * PUT and POST actions to the write permission. The correspondence is not
	 * exact however since a HTTP GET operation may cause data to be modified
	 * and a POST operation may cause modification to a resource other than the
	 * one specified in the request. For this reason a separate Action URI
	 ...
	 * <li><code>+4 if suid is set</code> The user group and world access
	 * permissions have the value
	 * <li><code>+1 if execute permission is granted</code>
	 * <li><code>+2 if write permission is granted</code>
	 * <li><code>+4 if read permission is granted</code> For example 0754
	 * denotes the UNIX file access permission: user read, write and execute,
	 * group read and execute and world read.
	 *
	 * @supported.api
	 */
	...
	/**
	 * Assertion handle, request id, and response id have this length. If server
	 * id cannot be found, assertion id has this length also. request id,
	 * response id, and assertion id will be base64 encoded for printing.
	 */
	 ...
	 * @supported.api
	 */
	public static String CONFIRMATION_METHOD_ARTIFACT = "urn:oasis:names:tc:SAML:1.0:cm:artifact";
	 ...
	 * @supported.api
	 */
	public static final String CONFIRMATION_METHOD_SENDERVOUCHES = "urn:oasis:names:tc:SAML:1.0:cm:sender-vouches";
	...
	/**
	 * SAML service attribute that specifies whether needs to remove assertion
	 * after it's being dereferenced or not.
	 */
	...
	/**
	 * SAML service attribute that specifies whether to sign an assertion or
	 * not.
	 */
	 ...
	 */
	public static final String NAME_ID_FORMAT_MAP = "iplanet-am-saml-name-id-format-attr-map";

	/**
	 * SAML service attribute that specifies the attribute map.
	 */
	public static final String ATTRIBUTE_MAP = "iplanet-am-saml-attr-map";
	...
	/**
	 * SAML service attribute that specifies list of trusted partners. A sample
	 * trusted server list entry is like target=.iplanet.com:8080|
	...
	/**
	 * SAML service attribute that specifies partner urls. A sample Parter URL
	 * list entry is like: SourceID=encoded 20 bytes|
	...
	/**
	 * A SAML service attribute parameter that defines the URL that provides the
	 * SAML service. The servlet specified in the URL implements the Web-browser
	 * SSO with Artifact profile defined in the OASIS-SAML Bindings and Profiles
	 * specification.
	 *
	 * @supported.api
	 */
	 ...
	 * used in SAML. It should be one of the following: NOAUTH BASICAUTH SSL
	 * SSLWITHBASICAUTH This parameter is optional, and if not specified, the
	 * default is NOAUTH. If BASICAUTH or SSLWITHBASICAUTH is specified, the
	 * User parameter is require and the SOAPUrl should be HTTPS.
	 *
	 * @supported.api
	 */
	...
	/**
	 * A SAML service attribute parameter that specifies a pluggable class which
	 * defines how the subject of an Assertion is related to an identity at the
	 * destination site. By default, it is:
	 ...
	 * A SAML service attribute parameter that specifies a pluggable class which
	 * defines how the subject of an Assertion is related to an identity at the
	 * destination site and a target URL. By default, it is:
	...
	/**
	 * A SAML service attribute parameter that specifies the class with the path
	 * where the siteAttributeMapper is located. Applications can develop a
	 * siteAttributeMapper to obtain attributes to be included in the assertion
	 * during SSO. If no siteAttributeMapper is found, then no attributes will
	 * be included in the assertion during SSO.
	 *
	 * @supported.api
	 */
	public static final String SITEATTRIBUTEMAPPER = "siteattributemapper";
	public static final String SITEATTRIBUTEMAPPERDEFAULT = "com.sun.identity.saml.plugins.DefaultSiteAttributeMapper";

	/**
	 * A SAML service attribute parameter that specifies the class with the path
	 * where the partnerSiteAttributeMapper is located. Applications can develop
	 * a partnerSiteAttributeMapper to obtain attributes to be included in the
	 * assertion during SSO based on target URL. If no
	...
	/**
	 * A SAML service attribute parameter that specifies the class with the path
	 * where the nameIdentifierMapper is located. Applications can develop a
	 * nameIdentifierMapper to obtain a name identifier to be included in the
	 * assertion during SSO. If no nameIdentifierMapper is found, then a default
	...
	/**
	 * A SAML service attribute parameter that lists the IP addresses and/or the
	 * certAlias for all of the hosts, within the specified partner site, that
	 * can send requests to this site. This ensures that the requester is indeed
	 * the intended receiver for the SAML artifact.
	 *
	 * @supported.api
	 */
	 ...
	 * for verifying the signature in an assertion, when the assertion is signed
	 * by a partner and the certificate of the partner can not be found in the
	 * KeyInfo portion of the signed assertion.
	 *
	 * @supported.api
	 */
	 ...
	 * assertion generated within OpenSSO. The syntax is hostname:port.
	 *
	 * @supported.api
	 */
	...
	/**
	 * A SAML service attribute parameter that specifies the class with the path
	 * to where the attributeMapper is located. Applications can develop an
	 * attributeMapper to obtain either an Session ID or an assertion containing
	 * AuthenticationStatement from the query. The mapper is then used to
	 * retrieve the attributes for the subject. If no attributeMapper is
	 * specified, DefaultAttributeMapper will be used.
	 *
	 * @supported.api
	 */
	...
	/**
	 * A SAML service attribute parameter that specifies the class with the path
	 * to where the actionMapper is located. Applications can develop an
	 ...
	 * query. If no actionMapper is specified, DefaultActionMapper will be used.
	 *
	 * @supported.api
	 */
	...
	/**
	 * SAML service attribute that specifies maximum number of assertions the
	 * server can have at a given time.
	 */
	...
	/**
	 * System property name that specifies cleanup interval for internal cache,
	 * such as assertions, artifacts and keystore etc. This property is mapped
	 * to the CLEANUP_INTERVAL_NAME attribute in SAML service on the server
	 * side.
	 */
	public static final String CLEANUP_INTERVAL_PROPERTY = "com.sun.identity.saml.cleanup.interval";
	...
	public static final String JKS_KEY_PROVIDER = "com.sun.identity.saml.xmlsig.JKSKeyProvider";
	public static final String KEY_PROVIDER_IMPL_CLASS = "com.sun.identity.saml.xmlsig.keyprovider.class";
	public static final String SIGNATURE_PROVIDER_IMPL_CLASS = "com.sun.identity.saml.xmlsig.signatureprovider.class";
	public static final String AM_SIGNATURE_PROVIDER = "com.sun.identity.saml.xmlsig.AMSignatureProvider";
	...
	/**
	 * Attribute that identifies server host in <code>AMConfig.properties</code>
	 * file.
	 */
	public static final String SERVER_HOST = "com.iplanet.am.server.host";

	/**
	 * Attribute that identifies server port in <code>AMConfig.properties</code>
	 * file.
	 */
	 ...
	 * file.
	 */
	public static final String SERVER_URI = "com.iplanet.am.services.deploymentDescriptor";
	 ...
	 * @supported.api
	 */
	public static final String TRANSFORM_BASE64_DECODE = XMLSIG_NAMESPACE_URI + "base64";
	 ...
	 * @supported.api
	 */
	public static final String TRANSFORM_ENVELOPED_SIGNATURE = XMLSIG_NAMESPACE_URI + "enveloped-signature";
	...
	/**
	 * Attribute which identifies certificate alias of this site. This value is
	 * configured through <code>AMConfig.properties</code>.
	 */
	 ...
	 */
	public static final String ATTR_NAME_SPACE = "urn:com:sun:identity:attrnamespace";
	 ...
	 * If true, escaping special characters. Otherwise, will not. Default value
	 * is "true".
	 */
	public static final String ESCAPE_ATTR_VALUE = "com.sun.identity.saml.escapeattributevalue";
	 ...
	 */
	public static final String ERROR_PAGE_HTTP_BINDING = "com.sun.identity.saml.errorpage.httpbinding";

	/**
	 * Property to identify the error page url.
	 */
	public static final String ERROR_PAGE_URL = "com.sun.identity.saml.errorpage.url";
	/**
	 * Default error page url.
	 */
	public static final String DEFAULT_ERROR_PAGE_URL = "/saml2/jsp/saml2error.jsp";
	/**
	 * HTTP status code.
	 */

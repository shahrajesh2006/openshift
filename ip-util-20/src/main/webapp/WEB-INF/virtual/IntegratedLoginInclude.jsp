...
<form:form action="ILDirectorAction" method="post" style="margin:0px;"
	...
	<%
		//Default to display the link mouseover text as primary login id, for OMA
				String title = "Primary Login ID: " + urlObject.parameters.get("userName");
				if (urlObject.parameters.get("agentUserName") != null) {
					//But if this is an agent sso login, show the agent user name
					title = "Agent User Name: " + urlObject.parameters.get("agentUserName");
				} else if (urlObject.parameters.get("tokenType") != null
						&& ((String) urlObject.parameters.get("tokenType")).equalsIgnoreCase("XML")) {
					//For an xml token, we may not have a username parameter, but rather just an account Number
					if (urlObject.parameters.get("accountNumber") != null) {
						title = "Account Number: " + urlObject.parameters.get("accountNumber");
					} else {
						//If there is not account number, assume it is a visitor login and display the attribute
						title = "Attribute: " + urlObject.parameters.get("marketSegmentOne");
					}
				}
	%>
	<a href="#"
	...
	| <span
		style="cursor: pointer; color: -webkit-link; text-decoration: underline;"
		onclick="document.getElementById('d_<%=urlObject.hashCode()%>').style.display = 'inline';">change?</span>

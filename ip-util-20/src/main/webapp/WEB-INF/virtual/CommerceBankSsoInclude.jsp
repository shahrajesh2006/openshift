...
%>
	...
	<form:form action="CommerceBankSsoAction" method="post" style="margin:0px;" id='<%=("f_" + urlObject.hashCode())%>' styleId='<%= ("f_" + urlObject.hashCode()) %>'>
...
%>
	...
	| <a href="#" onclick="document.getElementById('d_<%= urlObject.hashCode() %>').style.display = 'inline';">change?</a>
...
<% if (isAdmin) { %>
	...
	<form:form action="CommerceBankSsoAction"  method="post" style="margin:0px;">
	<input type="hidden" property="method" value="postValues" />

	<table border="0" style="margin: 4px; border-width: 1px; border-color:gray; border-style:solid;">

	<tr><td colspan="2">You can change any of the values below.</td></tr>
...
<%
	for (Iterator ii = urlObject.parameters.entrySet().iterator(); ii.hasNext();)
    {
        ...
        out.println("<td><input type='text' name='" + e.getKey() + "' value='" + e.getValue() + "' size='70'></td>");
...
%>
	...
	<tr><td colspan="2" align="right"><input type="submit"
					value="Submit"></td>
	<td>
	<input type="button" onclick="document.getElementById('d_<%= urlObject.hashCode() %>').style.display = 'none';" value="Close">
